...
HRESULT CParticleSystem::Render(float tFrame)
{
	...
	DWORD PointSprite,PointScale,ZWrite,AlphaBlend,Light;
	m_pDevice->GetRenderState(D3DRS_POINTSPRITEENABLE,&PointSprite);
	m_pDevice->GetRenderState(D3DRS_POINTSCALEENABLE,&PointScale);
	...
	// Reset render states
	m_pDevice->SetRenderState(D3DRS_POINTSPRITEENABLE,PointSprite);
	m_pDevice->SetRenderState(D3DRS_POINTSCALEENABLE,PointScale);
	m_pDevice->SetRenderState(D3DRS_ZWRITEENABLE,ZWrite);
